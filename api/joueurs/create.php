...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($ba_bec_errors)) {
        ...
        sql_insert(
            'JOUEUR',
            'prenomJoueur, nomJoueur, urlPhotoJoueur, posteJoueur, numMaillot, anneeArrivee, clubsPrecedents, dateNaissance',
            "'$ba_bec_prenomJoueur', '$ba_bec_nomJoueur', $ba_bec_photoValue, $ba_bec_posteValue, $ba_bec_numeroValue, $ba_bec_anneeValue, $ba_bec_clubsValue, $ba_bec_dateValue"
        );
        $ba_bec_lastJoueur = sql_select('JOUEUR', 'numJoueur', null, null, 'numJoueur DESC', '1')[0]['numJoueur'];
        if ($ba_bec_lastJoueur && $ba_bec_numEquipe !== '' && is_numeric($ba_bec_numEquipe)) {
            sql_insert('EQUIPE_JOUEUR', 'numEquipe, numJoueur', "'$ba_bec_numEquipe', '$ba_bec_lastJoueur'");
        }
